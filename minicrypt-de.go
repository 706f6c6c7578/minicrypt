...
func validatePublicKey(content string) error {
    ...
    if len(block.Bytes) != ed25519.PublicKeySize {
        return fmt.Errorf("ungültige Schlüsselgröße: %d Bytes (erwartet %d)", 
...
func signMessage(keyPath string, r io.Reader, w io.Writer) error {
    ...
    if privKey.Size() != ed25519.PrivateKeySize {
        return errors.New("Ungültige Größe des geheimen Schlüssel.")
    }
...
func verifyMessage(r io.Reader, w io.Writer) error {
	...
	for scanner.Scan() {
		...
		if line == signatureMarker {
			if inSigBlock {
				return errors.New("Unerwarteter zweiter Signatur-Marker gefunden.")
			}
			inSigBlock = true
			dataCollected = true
			continue
	...
	if err := scanner.Err(); err != nil {
		return fmt.Errorf("Fehler beim Lesen der Eingabe zur Verifizierung.")
	}
	...
	if len(pubKeyHex) != ed25519PublicKeyHexLength {
		return fmt.Errorf("Ungültige Länge des öffentlichen Schlüssels.")
	}
	...
	if len(sigHex) != ed25519SignatureHexLength {
		return fmt.Errorf("Ungültige Gesamtlänge der Signatur.")
	}
	...
	if len(pubKey) != ed25519.PublicKeySize {
		return fmt.Errorf("Ungültige Größe des öffentlichen Schlüssels.")
	}
	...
	if len(signature) != ed25519.SignatureSize {
		return fmt.Errorf("Ungültige Größe der Signatur.")
	}
	...
	if err != nil {
		return fmt.Errorf("Verifizierungsergebnis konnte nicht geschrieben werden.")
	}
...
func processSPE(recipient string, paddingSize int, r io.Reader, w io.Writer) error {
    ...
    if err := pad(bytes.NewReader(signBuffer.Bytes()), paddingSize, &padBuffer); err != nil {
        return fmt.Errorf("padding fehlgeschlagen: %v", err)
    }
...
func processDUV(r io.Reader, w io.Writer) error {
	...
	if err != nil {
		return fmt.Errorf("Geheimer Schlüssel konnte nicht geladen werden.", err)
	}
	defer privKey.Destroy()
	if err := decrypt(privKey, r, &decryptBuffer); err != nil {
		return fmt.Errorf("Entschlüsselung fehlgeschlagen.", err)
	}

	unpaddedReader, err := unpad(bytes.NewReader(decryptBuffer.Bytes()))
	if err != nil {
		log.Printf("Zurücksetzen fehlgeschlagen, versuche Verifizierung der Rohdaten.")
	...
	if _, err := io.Copy(&verifyBuffer, unpaddedReader); err != nil {
		return fmt.Errorf("Daten konnten nicht in den Buffer kopiert werden.", err)
	}

    if _, err := w.Write(verifyBuffer.Bytes()); err != nil {
        return fmt.Errorf("Entschlüsselter Klartext konnte nicht geschrieben werden.", err)
    }

	if err := verifyMessage(bytes.NewReader(verifyBuffer.Bytes()), w); err != nil {
		return fmt.Errorf("Verifizierung fehlgeschlagen.", err)
	}
